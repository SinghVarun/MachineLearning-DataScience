...
theta(1)=0;
%shiftThetaDownward = theta(2:size(theta,1)); %theta(1) remains unchanged. 
...
J = (1/m)*(-transpose(y)* log(prediction) - transpose(1 - y)* log(1-prediction)) + (lambda/(2*m))*theta'*theta;
grad = ((1/m)*transpose(X)*(prediction - y)) + (lambda/m)*theta;
